...
Every runtime knob (config is env-only; there is no config file). Source of truth is
`internal/config/config.go`, plus two vars read directly by the GitLab slice.
Defaults marked **fail-closed** refuse
...
configuration knob may live under a reserved prefix.
...
in the DB or on the runner hosts carries the prefix; only reference _sites_
(scripts, inventories, host key fields) use the derived form.

**The KEK is app config, not a store secret**, so it lives outside the reserved
`CRONOMICON_SECRET_*` prefix as `CRONOMICON_KEK`, `CRONOMICON_KEK_FILE`,
`CRONOMICON_KEK_VERSION` and `CRONOMICON_KEK_<N>` / `_<N>_FILE` (see
**Stored-secret encryption** below).

### Writing reference sites

The derived form is the one to use; the server also resolves a bare row name,
but a bare name cannot be told from config at a glance, and the reference scan
behind the job bindings panel lints it as a site to move. Reference sites live in your
job-definitions repo and in SSH Targets:
...
single **test run** before writing the next. On a runner host, the agent's
prefixed→bare fallback lets an inventory reference `CRONOMICON_SECRET_<key>` even
when the host's own environment file (for example one rendered by a Vault Agent
sidecar) still exposes the bare `<key>`.
...
| `CRONOMICON_BOOTSTRAP_ADMIN_GROUP` | _(empty)_ | **First deploy only.** Any user in this group gets admin regardless of access grants. Logs a loud warning while set. Remove + redeploy after seeding real grants. |
| `CRONOMICON_LOGOUT_REDIRECT_URL` | _(empty)_ | **rec** Identity-provider logout endpoint the SPA navigates to on sign-out. |

**`CRONOMICON_TRUSTED_PROXIES` has a second job: it also gates client-IP attribution
...
> **Naming:** the KEK is **app config, not a store secret**, so it
...
Only once `--dry-run` reports rotation complete can the `_1` key be retired. A
hand-rolled `SELECT DISTINCT kek_version FROM secrets` is not a substitute: it
covers one of the three stores and would declare victory with the other two
...
| `CRONOMICON_RETENTION_LOG_FILES_DAYS` | `90` | Seeds `logFiles` — the **on-disk run-log** window — on first run only. **Operationally significant:** each nightly sweep reaps run logs older than this window. Set `0` (here before first boot, or in the panel afterwards) to keep them forever. |
...
shortening it below `changeLog` throws away the tamper-evident tail early. It
starts at the default; nothing seeds it from env. `0` = keep forever.
...
**The reaper walks the per-entity folders.** A run log lives at
...
describes. Deleting a job or workflow leaves its folder in place (the audit trail
...
| `CRONOMICON_BACKUP_S3_CA_FILE` | _(empty)_ | Optional PEM bundle for a backup bucket on a private S3 node signed by an internal CA. The log archive's equivalent is the pasted CA Bundle in Settings → Log Storage; both feed the same client. |

## Notifications

| Var | Default | Notes |
|---|---|---|
| `CRONOMICON_APPRISE_URL` | _(empty)_ | Apprise gateway base URL, e.g. `http://apprise:8000` (an Apprise sidecar). Used as the push gateway when an alert rule includes a push/apprise channel. The per-config gateway override (`apprise_url` in notification settings) takes precedence if set. |
...
## Vault — disabled unless configured
...
**Client hardening — each knob dormant until set:**
...
**Run-injection kill-switch:**

| Var | Default | Notes |
|---|---|---|
| `CRONOMICON_SECRETS_INJECTION_ENABLED` | `true` | Dispatch-time reference injection (bound `CRONOMICON_SECRET_/VAR_/KEY_*` resolved and injected into runs). Set false to hard-disable injection. Plural `SECRETS_` sits outside the reserved `CRONOMICON_SECRET_*` reference prefix, so it is config, not an injected value. |
...
timezone blobs). The precedence rule is applied uniformly:
