...
## Namespace (which `AMADEUS_*` names are config vs. references)

> **Rule:** If an `AMADEUS_*` name starts with `VAR_`, `SECRET_`, `KEY_`, or
> `RUN_`, it is a **reference** the app resolves and injects into runs. **Every
> other `AMADEUS_*` name is server/runner configuration** — that is, everything
> in the tables below.
...
|---|---|---|---|
| `AMADEUS_VAR_*` | Variables | user | plaintext **value** (log-safe) |
| `AMADEUS_SECRET_*` | Secrets (stored or Vault-backed — not distinguished) | user | sensitive **value** (always redacted) |
| `AMADEUS_KEY_*` | SSH Keys | user | **file path** to key material on the executing host |
| `AMADEUS_RUN_*` | dispatcher | Cronomicon (fixed set) | read-only run **context** (injected since v0.49.1) |

**The `AMADEUS_RUN_*` fixed set** (injected into every run by the executor; not
bindable, never from a store row — a run's own metadata, always log-safe):
`AMADEUS_RUN_ID` (run/trace id), `AMADEUS_RUN_JOB` (job name), `AMADEUS_RUN_JOB_SOURCE`
(`git`|`amadeus`), `AMADEUS_RUN_SCOPE` (`""` = global), `AMADEUS_RUN_TYPE` (run type),
`AMADEUS_RUN_TRIGGERED_BY` (actor), `AMADEUS_RUN_EXECUTOR` (`ssh`|`runner`). The SSH
...
prefixing — `reference = AMADEUS_<SECTION>_<row name, verbatim>` — and resolution
...
not a store secret, so it lives outside the reserved `AMADEUS_SECRET_*` prefix
as `AMADEUS_KEK`, `AMADEUS_KEK_FILE`, `AMADEUS_KEK_VERSION` and
`AMADEUS_KEK_<N>` / `_<N>_FILE`. The `AMADEUS_SECRET_KEK*` spellings dual-read
for a deprecation window (v0.48.0–v1.5.40) and were **removed in v1.5.41** — a
deployment still setting them gets "no KEK configured" at the first
stored-secret operation.

### Rollout (incremental, no flag-day)

The reference half can be adopted at your own pace — the server resolves both
bare and derived references. **The KEK half cannot**: the old spelling stopped
reading in v1.5.41, so step 1 is a prerequisite for that upgrade, not an option.

1. **Server KEK env (REQUIRED before upgrading to v1.5.41 or later).** Set
   `AMADEUS_KEK*`. The `AMADEUS_SECRET_KEK*` spelling no longer reads, and a
   deployment still on it starts but fails its first stored-secret operation.
2. **Reference sites (in your job-definitions repo + SSH Targets).** Move
   ...
   - **Scripts** → `AMADEUS_SECRET_<key>` / `AMADEUS_VAR_<key>` (copy the exact
     reference from the Env Vars page).
   - **Inventories** → `lookup('env','AMADEUS_KEY_<label>')` /
     `lookup('env','AMADEUS_SECRET_<key>')`.
   - **Host / bastion `authKeyEnvVar` fields** → `AMADEUS_KEY_<label>`.
...
3. **Out of scope here:** the Vault Agent sidecar's rendered `secrets.env` names
   ...
   fallback lets an inventory move to `AMADEUS_SECRET_<key>` even before the
   sidecar is touched.
...
| Var | Default | Notes |
|---|---|---|
| `AMADEUS_ADDR` | `:8080` | Listen address. Not published to the host (proxy-only). |
| `AMADEUS_LOG_LEVEL` | `info` | `debug\|info\|warn\|error`. |
| `AMADEUS_LOG_FORMAT` | `json` | `json\|text`. **rec** `json` in prod. |
| `AMADEUS_LOG_FILE_ENABLED` | `true` | Also write the **process log** to a file on disk. **stdout is always written** — the file is strictly additive, never a replacement, so journald/docker log collection is unaffected. `false` turns the file off entirely (stdout only). |
| `AMADEUS_LOG_FILE` | _(empty)_ | Absolute path of the process log. Empty ⇒ `amadeus.log` in the **run-log directory** (Settings → Log Storage), and it follows that directory when the setting changes. A **relative path is rejected at boot**. Setting it explicitly also pins the file *and* attaches it **earlier in boot** (before the DB opens), so the build banner and config warnings land in the file too; the derived default attaches just after migrations. Worth pinning if the run-log tree lives on slow/shared storage — a continuously appended file maps badly onto object storage. |
| `AMADEUS_LOG_FILE_MAX_MB` | `64` | Size at which the live file rotates. Must be **≥ 1**. |
| `AMADEUS_LOG_FILE_KEEP` | `5` | Rotated generations retained, named `amadeus.log.1` … `amadeus.log.N` (`.1` = newest). Must be **≥ 0**; `0` truncates in place with no generations kept. |
| `AMADEUS_AUDIT_LOG_ENABLED` | `true` | Write the **compliance audit stream** (`audit.log`) to disk. This is a *different* file from the process log: one JSON-Lines record per audited event, keys fixed by a versioned schema. `false` turns the file off entirely — audit rows are still written to the database, which is authoritative either way. |
| `AMADEUS_AUDIT_LOG` | _(empty)_ | Absolute path of the audit stream. Empty ⇒ `audit.log` in the **run-log directory** (Settings → Log Storage), and it follows that directory when the setting changes. A **relative path is rejected at boot**. |
| `AMADEUS_MAX_RUN_LOG_BYTES` | `536870912` | Cap on a single run's ingested log (512 MiB). The log-ingest endpoint is exempt from the 2 MiB body cap (runs stream many chunks); this bounds per-run growth so a rogue/compromised runner can't fill the disk. On reaching the cap, further ingest is refused with `413`. `0` disables the cap. |
| `AMADEUS_OUTBOUND_ALLOW_PRIVATE` | `true` | SSRF egress guard (SU-7) on operator-configured outbound targets (Vault, GitLab, S3/MinIO, Apprise, OIDC). Cloud-metadata / loopback / link-local are **always** blocked. `true` (default) permits RFC-1918 / unique-local — required here because Vault/GitLab are internal hosts. Set `false` only when every outbound target is public. The S3 IAM-role provider (IMDS) is exempt. |
| `AMADEUS_OUTBOUND_ALLOW_LOOPBACK` | `false` | Re-permit loopback outbound past the SSRF guard. Set `true` **only** for a Vault-agent loopback sidecar (Vault addr on `127.0.0.1`); it weakens the guard for every outbound client. |
...
run-log reaper (`AMADEUS_RETENTION_LOG_FILES_DAYS`, § Retention below) never sees
...
| Var | Default | Notes |
|---|---|---|
| `AMADEUS_DB_PATH` | `/var/lib/amadeus/amadeus.db` | SQLite file on the mounted volume. |
| `AMADEUS_GIT_CACHE_DIR` | `/var/lib/amadeus/git-cache/job-definitions` | GitLab clone cache. Keep on the volume so it survives restarts. |

## Auth — Trusted Header SSO (Phase A, primary)

| Var | Default | Notes |
|---|---|---|
| `AMADEUS_AUTH_MODE` | `trusted-header` | `trusted-header\|oidc`. |
| `AMADEUS_TRUSTED_PROXIES` | _(empty)_ | **req** in trusted-header mode — CIDR/IP allowlist of the reverse proxy. **Fail-closed: empty ⇒ refuse to boot** (unless `AMADEUS_DEV_AUTH`). In the compose stack this is Traefik's static internal IP, e.g. `172.28.0.2/32`. |
| `AMADEUS_TRUSTED_HEADER_USER` | `Remote-User` | Header-name override. |
| `AMADEUS_TRUSTED_HEADER_EMAIL` | `Remote-Email` | Header-name override. |
| `AMADEUS_TRUSTED_HEADER_NAME` | `Remote-Name` | Header-name override. |
| `AMADEUS_TRUSTED_HEADER_GROUPS` | `Remote-Groups` | Header-name override (comma-separated groups). |
| `AMADEUS_BOOTSTRAP_ADMIN_GROUP` | _(empty)_ | **First deploy only.** Any user in this group gets admin regardless of `ad_group_mappings`. Logs a loud warning while set. Remove + redeploy after seeding real mappings. |
| `AMADEUS_LOGOUT_REDIRECT_URL` | _(empty)_ | **rec** Identity-provider logout endpoint the SPA navigates to on sign-out. |

**`AMADEUS_TRUSTED_PROXIES` now has a second job: it also gates client-IP attribution
...
Only consulted when `AMADEUS_AUTH_MODE=oidc`.

| Var | Default | Notes |
|---|---|---|
| `AMADEUS_OIDC_ISSUER` | _(empty)_ | OIDC issuer URL. |
| `AMADEUS_OIDC_CLIENT_ID` | _(empty)_ | |
| `AMADEUS_OIDC_CLIENT_SECRET` | _(empty)_ | Provide via secret file/env, never committed. |
| `AMADEUS_OIDC_REDIRECT_URL` | _(empty)_ | |
| `AMADEUS_SESSION_HASH_KEY` | _(empty)_ | base64, 32+ bytes — signs the session cookie. **Required in oidc mode** for sessions to survive restart (ephemeral random key otherwise). Irrelevant in trusted-header mode. |
| `AMADEUS_SESSION_BLOCK_KEY` | _(empty)_ | base64, 16/24/32 bytes — encrypts the session cookie. |

## Cookies / TLS

| Var | Default | Notes |
|---|---|---|
| `AMADEUS_COOKIE_SECURE` | `true` | **req** `true` in prod (HTTPS via the proxy). |

## Developer bypass (NEVER in production)

| Var | Default | Notes |
|---|---|---|
| `AMADEUS_DEV_AUTH` | `false` | **dev** One-click synthetic-admin login, no identity provider. Also exempts the trusted-proxies boot check. Leave unset in prod. |
| `AMADEUS_DEV_SEED` | `false` | **dev** Seeds demo data into an empty DB. Leave unset in prod. |

## Integrations

| Var | Default | Notes |
|---|---|---|
| `AMADEUS_GITLAB_BASE_URL` | _(empty)_ | GitLab repo URL for job-definition sync (B3). |
| `AMADEUS_GITLAB_TOKEN` | _(empty)_ | Read token for private repos; unauthenticated clone if empty. Secret. |
| `AMADEUS_GITLAB_WEBHOOK_SECRET` | _(empty)_ | Validates `X-Gitlab-Token` on the webhook. Secret. |
| `AMADEUS_GITLAB_WRITE_BRANCH` | _(empty)_ | The GitOps branch used for **both** sync-read and publish-write (V1.1-10) — there is one branch, not a read/write pair. Empty ⇒ fall back to the DB-backed `gitlab_config.write_branch` (**Settings → GitLab**), then `main`. Resolved **fresh per operation**, so a DB-side change applies without a restart; setting it here pins the branch and the panel value is ignored. |
| `AMADEUS_RUNNER_BOOTSTRAP_TOKEN` | _(empty)_ | Out-of-band bootstrap registration token (T9/A6.1). Multi-use, env-configured; unlike UI-minted tokens (single-use per install since v0.47.4) it is never consumed. Secret. |
| `AMADEUS_AGENT_DIR` | `/usr/share/amadeus/agents` | Directory of runner-agent binaries + `SHA256SUMS` served unauthenticated at `GET /agents/{filename}` (provisioning D1; the container image bakes them in). Missing dir ⇒ clean 404 with a build-it-yourself hint — bare-metal deploys can point this at their own build output. |
...
| Var | Default | Notes |
|---|---|---|
| `AMADEUS_RUNNER_OFFLINE_AFTER` | `5m` | Go duration. An `online`/`draining` runner whose last heartbeat is older than this is marked **offline**. Agents poll once a minute (D7), so the default is ≈5 missed polls. Shortening it below the poll interval will flap runners offline between polls. |
| `AMADEUS_RUNNER_DEREGISTER_AFTER` | `336h` (14 days) | Go duration. A runner that stays **offline** this long is fully removed — its tokens revoked and its row deleted (D4). "Offline since" is derived from `last_seen_at` (or `registered_at` when it never polled), so this window is measured from the last heartbeat, not from the moment it was marked offline. A host that comes back later must re-register. |
...
| Var | Default | Notes |
|---|---|---|
| `AMADEUS_SSH_EXECUTOR_ENABLED` | `false` | Master switch for the in-process SSH worker pool. `false` ⇒ the claim loop and the periodic orphan reaper never start and ssh runs stay queued. Note the **startup orphan sweep runs either way**: an `executor='ssh'` run left `running` by a crash holds a global concurrency slot until it is reconciled, so it is reconciled to `executor_lost` at boot whether or not the executor is enabled. |
| `AMADEUS_SSH_EXECUTOR_CONCURRENCY` | `4` | Simultaneous ssh runs claimed by this process. `≤ 0` is treated as the default rather than "unbounded" or "none". Within a single run, fan-out across targets is separately bounded (EX-D5). |
| `AMADEUS_SSH_EXECUTOR_STALE_AFTER` | `24h` | Go duration bounding the **periodic** orphan reaper (PP-H2), which runs once a minute while the executor is enabled: an `executor='ssh'` run still `running` longer than this is reconciled to failure (`executor_lost`). Keep it safely **above the longest plausible job** (its A12 timeout) — this window is a safety net, not the crash-recovery path, and a value below a real job's runtime will kill live runs. `≤ 0` falls back to the default. |

## Stored-secret encryption (S14)

> **Naming:** the KEK is **app config, not a store secret**, so as of v0.48.0 it
> lives under `AMADEUS_KEK*` — out of the reserved `AMADEUS_SECRET_*` reference
> prefix (see **Namespace** above). The old `AMADEUS_SECRET_KEK*` names were a
> dual-read alias through v1.5.40 and are gone.

| Var | Default | Notes |
|---|---|---|
| `AMADEUS_KEK_FILE` | _(empty)_ | **rec** Mounted file holding the base64 KEK (preferred over env). **Back up SEPARATELY from the SQLite S3 backup** — losing it makes stored secrets unrecoverable. |
| `AMADEUS_KEK` | _(empty)_ | Fallback: base64 KEK supplied directly. File takes precedence if both set. |
| `AMADEUS_KEK_VERSION` | `1` | Active KEK version written into each newly created/updated secret. Bump when rotating (see below). |
| `AMADEUS_KEK_<N>` / `AMADEUS_KEK_<N>_FILE` | _(empty)_ | Historical KEK for version `N`, supplied during/after a rotation so secrets still wrapped under the old key remain decryptable. Same base64/file rules as the active KEK. |

**KEK rotation (zero-downtime):** to rotate from version 1 to 2 — (1) move the
current key to `AMADEUS_KEK_1` (or `_1_FILE`), (2) set the new key in
`AMADEUS_KEK`/`AMADEUS_KEK_FILE` and `AMADEUS_KEK_VERSION=2`,
...
account the server runs as. An env-supplied `AMADEUS_KEK` has no mode and is not
...
| Var | Default | Notes |
|---|---|---|
| `AMADEUS_RETENTION_RUNS_DAYS` | `90` | Seeds `runs`/`activity`/`workflowRuns` on **first run only**; the stored blob is authoritative thereafter. |
| `AMADEUS_RETENTION_CHANGELOG_DAYS` | `365` | Seeds `changeLog`/`schedulePushes` on **first run only**; the stored blob is authoritative thereafter. |
| `AMADEUS_RETENTION_LOG_FILES_DAYS` | `90` | Seeds `logFiles` — the **on-disk run-log** window — on first run only. **Operationally significant:** before this knob nothing ever deleted log files, so after upgrading, the first nightly sweep reaps run logs older than this window. Set `0` (here before first boot, or in the panel afterwards) to keep them forever. |
...
it. `AMADEUS_BACKUP_AT` therefore also decides when retention runs.

| Var | Default | Notes |
|---|---|---|
| `AMADEUS_BACKUP_AT` | `02:00` | Daily **wall-clock UTC** `HH:MM` for the retention + backup sweep (PP-H6). Always UTC — *not* the application timezone from Settings → General, which governs scheduling and display only. An unset or unparseable value falls back to `02:00` rather than failing to boot. Wall-clock anchoring (rather than a 24h-from-boot ticker) is deliberate: a process restarted more often than daily would never have reached the end of a 24h ticker, so a frequently-redeployed deployment could go indefinitely without a durable backup. A **boot catch-up** sweep also runs immediately when the last recorded success is more than 24h old, gated on that persisted timestamp so a crash-loop still backs up at most about once a day. |
| `AMADEUS_BACKUP_S3_BUCKET` | _(empty)_ | Empty ⇒ local snapshots only (no upload). |
| `AMADEUS_BACKUP_S3_ENDPOINT` | _(empty)_ | `host:port`; empty ⇒ AWS S3. Set for MinIO. |
| `AMADEUS_BACKUP_S3_REGION` | `us-east-1` | |
| `AMADEUS_BACKUP_S3_ACCESS_KEY` | _(empty)_ | Secret. |
| `AMADEUS_BACKUP_S3_SECRET_KEY` | _(empty)_ | Secret. |
| `AMADEUS_BACKUP_S3_USE_SSL` | `true` | |
| `AMADEUS_BACKUP_S3_CA_FILE` | _(empty)_ | Optional PEM bundle for a backup bucket on a private S3 node signed by an internal CA (SL band). The log archive's equivalent is the pasted CA Bundle in Settings → Log Storage; both feed the same client. |

## Notifications (Phase C.1)

| Var | Default | Notes |
|---|---|---|
| `AMADEUS_APPRISE_URL` | _(empty)_ | Apprise gateway base URL, e.g. `http://apprise:8000` (the Phase B sidecar). Used as the push gateway when an alert rule includes a push/apprise channel. The per-config gateway override (`apprise_url` in notification settings) takes precedence if set. |
...
> is required to use authenticated SMTP — see `AMADEUS_KEK_FILE`).
...
The Vault client (AppRole, KV v2) stays a no-op stub unless `AMADEUS_VAULT_ADDR`
...
`deploy/vault-runbook.md`.

| Var | Default | Notes |
|---|---|---|
| `AMADEUS_VAULT_ADDR` | _(empty)_ | Vault base URL, e.g. `https://vault.internal:8200`. Empty ⇒ Vault disabled. |
| `AMADEUS_VAULT_ROLE_ID` | _(empty)_ | AppRole role id. |
| `AMADEUS_VAULT_ROLE_ID_FILE` | _(empty)_ | Mounted file with the role id (preferred; overrides the inline value). |
| `AMADEUS_VAULT_SECRET_ID` | _(empty)_ | AppRole secret id. Secret. |
| `AMADEUS_VAULT_SECRET_ID_FILE` | _(empty)_ | Mounted file with the secret id (preferred). |

**Client hardening (Phase 2, D4) — each knob dormant until set; unset = prior behavior:**

| Var | Default | Notes |
|---|---|---|
| `AMADEUS_VAULT_NAMESPACE` | _(empty; DB `vault_config.namespace`)_ | Sent as `X-Vault-Namespace` on every request (Vault Enterprise / HCP). Env overrides the DB value. |
| `AMADEUS_VAULT_CA_FILE` | _(empty)_ | PEM CA bundle; pins the client transport instead of system roots. Missing/invalid ⇒ **fails loud** (Vault stays disabled), never a silent system-root fall-back. |
| `AMADEUS_VAULT_SECRET_ID_WRAPPED` | `false` | When true, the secret id is a single-use response-wrapping token, unwrapped once via `sys/wrapping/unwrap` and cached. Static long-lived secret id is the default. |
...
| Var | Default | Notes |
|---|---|---|
| `AMADEUS_SECRETS_INJECTION_ENABLED` | `true` | Dispatch-time reference injection (bound `AMADEUS_SECRET_/VAR_/KEY_*` resolved and injected into runs). Set false to hard-disable for a release. Plural `SECRETS_` sits outside the reserved `AMADEUS_SECRET_*` reference prefix, so it is config, not an injected value. |
...
|---|---|---|---|
| GitLab repo URL / PAT | `AMADEUS_GITLAB_BASE_URL` / `AMADEUS_GITLAB_TOKEN` | `gitlab_config` | restart |
| GitLab webhook secret | `AMADEUS_GITLAB_WEBHOOK_SECRET` (pins validation; **rotation API returns 409 while set**) | `gitlab_config` (+ rotation overlap pair) | restart |
| GitLab write branch | `AMADEUS_GITLAB_WRITE_BRANCH` | `gitlab_config.write_branch` | next sync/publish (read per operation) |
| Vault addr / AppRole | `AMADEUS_VAULT_ADDR` / `AMADEUS_VAULT_ROLE_ID[_FILE]` / `AMADEUS_VAULT_SECRET_ID[_FILE]` | `vault_config` | restart |
| Vault namespace | `AMADEUS_VAULT_NAMESPACE` | `vault_config.namespace` | restart |
...
| Retention windows | seeded from `AMADEUS_RETENTION_*` on first boot only | `auditCompliance` settings blob | next nightly sweep |
