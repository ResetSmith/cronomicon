...
// `ORDER BY source LIMIT 1` (alphabetical — 'amadeus' always won) while the
...
func TestStepSourceOrder(t *testing.T) {
	...
	if got := workflow.StepSourceOrder("", "amadeus"); !eq(got, []string{"amadeus", "git"}) {
		t.Errorf("amadeus workflow order = %v, want [amadeus git]", got)
	}
	// The case alphabetical resolution got wrong.
	if got := workflow.StepSourceOrder("", "git"); !eq(got, []string{"git", "amadeus"}) {
		t.Errorf("git workflow order = %v, want [git amadeus] — alphabetical order would say the opposite", got)
	}
	// An explicit per-step override is absolute: no fallback to the other pool.
	if got := workflow.StepSourceOrder("git", "amadeus"); !eq(got, []string{"git"}) {
