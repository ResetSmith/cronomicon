...
// row and an operator (amadeus) row, the amadeus row wins, and HostByName carries
...
func TestHostByNamePrecedence(t *testing.T) {
	...
	// A scope 'prod' with a git-imported host row + an operator (amadeus) overlay
	...
	exec(`INSERT INTO ssh_hosts(id,source,hostname,address,port,username,auth_key_env_var,created_at,last_modified_at)
	      VALUES('am1','amadeus','web1','10.9.9.9',2222,'opuser','OP_KEY','t','2026-01-02T00:00:00Z')`)
	...
	if tgt.ID != "am1" || tgt.User != "opuser" || tgt.AuthKeyEnvVar != "OP_KEY" || tgt.Port != 2222 || tgt.Address != "10.9.9.9" {
		t.Errorf("precedence resolved %+v, want the amadeus overlay (am1/opuser/OP_KEY/2222)", tgt)
	}
	...
	// (with no amadeus overlay) returns nothing.
	...
	// Deterministic tiebreak: two amadeus rows, same hostname, identical
	// last_modified_at → a stable pick (id DESC), not query-plan luck.
	exec(`INSERT INTO ssh_hosts(id,source,hostname,created_at,last_modified_at) VALUES('amA','amadeus','dup','t','2026-02-01T00:00:00Z')`)
	exec(`INSERT INTO ssh_hosts(id,source,hostname,created_at,last_modified_at) VALUES('amB','amadeus','dup','t','2026-02-01T00:00:00Z')`)
	...
	// Cronomicon SCOPE isolation (M5): two amadeus scopes that both imported a host
	// named 'shared' must each resolve THEIR OWN row — a scoped amadeus import is
	// not a global overlay. A NULL-scope operator overlay still wins for both.
	exec(`INSERT INTO scopes(id,name,source,created_at) VALUES('sa','scopeA','amadeus','t')`)
	exec(`INSERT INTO scopes(id,name,source,created_at) VALUES('sb','scopeB','amadeus','t')`)
	exec(`INSERT INTO ssh_hosts(id,source,scope_id,hostname,address,created_at,last_modified_at) VALUES('ha','amadeus','sa','shared','10.0.0.1','t','2026-01-01T00:00:00Z')`)
	exec(`INSERT INTO ssh_hosts(id,source,scope_id,hostname,address,created_at,last_modified_at) VALUES('hb','amadeus','sb','shared','10.0.0.2','t','2026-01-02T00:00:00Z')`)
	...
	exec(`INSERT INTO ssh_hosts(id,source,hostname,address,created_at,last_modified_at) VALUES('ov','amadeus','shared','10.9.9.9','t','2025-01-01T00:00:00Z')`)
	...
	exec(`INSERT INTO ssh_hosts(id,source,hostname,created_at) VALUES('bare','amadeus','bareh','t')`)
