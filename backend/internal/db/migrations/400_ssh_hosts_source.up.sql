...
-- dials them with zero operator action, while keeping operator (amadeus) rows
-- authoritative. `source` mirrors the git|amadeus dual-source model (D4);
-- `scope_id`+`synced_at` let sync prune imported rows by owner without ever
-- touching operator rows. Existing rows default to 'amadeus' (operator-authored).
--
...
ALTER TABLE ssh_hosts ADD COLUMN source TEXT NOT NULL DEFAULT 'amadeus'
    CHECK (source IN ('git','amadeus'));
ALTER TABLE ssh_hosts ADD COLUMN scope_id TEXT;   -- inventory scope an imported (git) row came from; NULL for operator rows
ALTER TABLE ssh_hosts ADD COLUMN synced_at TEXT;  -- stamped on import; NULL for amadeus rows (prune key)
