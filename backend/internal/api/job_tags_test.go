...
// TestUpdateJobTags exercises the Jobs tags write API (tags-support.md D6): the
// CSRF guard, a valid full-replace with normalization, the length/count/control
// caps, the missing-field 422, the unknown-job 404, that the tags surface on both
// the detail and list reads, and that an empty array clears them. Jobs are keyed by
// integer rowid (source-agnostic).
func TestUpdateJobTags(t *testing.T) {
	...
	if _, err := pool.ExecContext(ctx,
		`INSERT INTO jobs(name, source, run_type, command, scope, content_hash, source_path, synced_at)
		 VALUES('backup-db','git','bash','pg_dump mydb','Prod','sha256:aaa','jobs/backup-db.yaml','t')`); err != nil {
		t.Fatalf("seed: %v", err)
	}
	var rowid int64
	if err := pool.QueryRowContext(ctx, `SELECT rowid FROM jobs WHERE name='backup-db'`).Scan(&rowid); err != nil {
		t.Fatalf("rowid: %v", err)
	}
	id := strconv.FormatInt(rowid, 10)

	client, csrf := devLoginWithCSRF(t, ts)
	put := func(jobID string, body any, hdrCSRF bool) *http.Response {
		t.Helper()
		b, _ := json.Marshal(body)
		req, _ := http.NewRequest(http.MethodPut, ts.URL+"/api/v1/job-tags/"+jobID, bytes.NewReader(b))
	...
	resp = put(id, map[string]any{"tags": []string{" Prod ", "db", "", "PROD"}}, true)
	...
	var raw string
	_ = pool.QueryRow(`SELECT tags FROM jobs WHERE name='backup-db'`).Scan(&raw)
	if raw != `["Prod","db"]` {
	...
	resp = put(id, map[string]any{"tags": []string{"ok\ttab"}}, true)
	resp.Body.Close()
	if resp.StatusCode != http.StatusUnprocessableEntity {
