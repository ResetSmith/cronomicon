...
func TestValidateSecrets(t *testing.T) {
	cases := []struct {
		...
		{name: "referencing an CRONOMICON_ name via lookup is allowed",
			content: "db1 ansible_become_pass=\"{{ lookup('env','CRONOMICON_SECRET_DB_PW') }}\"\n"},
		{name: "literal CRONOMICON_ assignment (standalone) is rejected",
			content:      "[db:vars]\nCRONOMICON_SECRET_X=hunter2\n",
			wantRejected: true, wantLine: 2, wantVar: "CRONOMICON_SECRET_X"},
		{name: "literal CRONOMICON_ assignment (inline host var) is rejected",
			content:      "db1 ansible_host=10.0.0.1 CRONOMICON_KEY_deploy=/tmp/k\n",
			wantRejected: true, wantLine: 1, wantVar: "CRONOMICON_KEY_deploy"},
		{name: "defining an CRONOMICON_ key even to a lookup value is rejected",
			content:      "CRONOMICON_VAR_X=\"{{ lookup('env','X') }}\"\n",
			wantRejected: true, wantLine: 1, wantVar: "CRONOMICON_VAR_X"},
	}
