...
// an AMADEUS_KEY_<bare> reference as the bare key-map/key-dir name (W3): files and
...
func TestResolveKeyPathStripsAmadeusKeyPrefix(t *testing.T) {
	...
	if p, src, ok := resolveKeyPath(nil, dir, "AMADEUS_KEY_ansible_rh8_key"); !ok || p != keyPath || src != "key-dir" {
	...
	if p, src, ok := resolveKeyPath(map[string]string{"deploy": "/keys/deploy"}, "", "AMADEUS_KEY_deploy"); !ok || p != "/keys/deploy" || src != "key-map" {
...
// value reference (AMADEUS_SECRET_/AMADEUS_VAR_) that is absent under its prefixed
...
func TestBuildChildEnvPrefixedToBareFallback(t *testing.T) {
	m := &runnerproto.ManifestResponse{RunType: "ansible", EnvPassthrough: []string{"AMADEUS_SECRET_DB_PW"}}
	out, prov, err := buildChildEnv(m, Config{}, []string{"DB_PW=s3cr3t"}, nil)
	if err != nil {
		t.Fatalf("buildChildEnv: %v", err)
	}
	if !envHas(out, "AMADEUS_SECRET_DB_PW", "s3cr3t") {
		t.Fatalf("want AMADEUS_SECRET_DB_PW=s3cr3t via bare fallback, got %v", out)
	}
	var sawProv bool
	for _, p := range prov {
...
func TestBuildChildEnvPrefixedWinsOverBare(t *testing.T) {
	m := &runnerproto.ManifestResponse{RunType: "ansible", EnvPassthrough: []string{"AMADEUS_SECRET_X"}}
	out, _, err := buildChildEnv(m, Config{}, []string{"AMADEUS_SECRET_X=prefixed", "X=bare"}, nil)
	if err != nil {
		t.Fatalf("buildChildEnv: %v", err)
	}
	if !envHas(out, "AMADEUS_SECRET_X", "prefixed") {
		t.Fatalf("prefixed value must win, got %v", out)
	}
}

// TestBuildChildEnvKeyReferenceResolvesToPath: an AMADEUS_KEY_<bare> passthrough
...
func TestBuildChildEnvKeyReferenceResolvesToPath(t *testing.T) {
	...
	m := &runnerproto.ManifestResponse{RunType: "ansible", EnvPassthrough: []string{"AMADEUS_KEY_mykey"}}
	out, _, err := buildChildEnv(m, Config{KeyDir: dir}, nil, nil)
	if err != nil {
		t.Fatalf("buildChildEnv: %v", err)
	}
	if !envHas(out, "AMADEUS_KEY_mykey", keyPath) {
		t.Fatalf("want AMADEUS_KEY_mykey=%s, got %v", keyPath, out)
	}
}
