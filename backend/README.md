...
| Var | Default | Notes |
|---|---|---|
| `AMADEUS_ADDR` | `:8080` | listen address |
| `AMADEUS_DB_PATH` | `/var/lib/amadeus/amadeus.db` | SQLite file (mounted volume, S13) |
| `AMADEUS_LOG_LEVEL` / `_FORMAT` | `info` / `json` | |
| `AMADEUS_LOG_FILE_ENABLED` / `AMADEUS_LOG_FILE` / `_MAX_MB` / `_KEEP` | `true` / — / `64` / `5` | Process log written to disk **in addition to stdout** (stdout is never replaced). Empty `AMADEUS_LOG_FILE` ⇒ `amadeus.log` in the run-log directory, following it live when that setting changes; an explicit path must be **absolute** and attaches earlier in boot (captures the build banner + config warnings). Rotates at `_MAX_MB` keeping `_KEEP` generations (`amadeus.log.1` … `.N`), so disk ≤ `(KEEP+1) × MAX_MB`. **Not** governed by `_RETENTION_LOG_FILES_DAYS`, and **not** redacted — see `deploy/security-review.md`. |
| `AMADEUS_AUDIT_LOG_ENABLED` / `AMADEUS_AUDIT_LOG` | `true` / — | Compliance **audit stream** (`audit.log`) — a separate file from the process log: one JSON-Lines record per audited event (`change_log`, a named subset of `activity`, and every auth event), keys fixed by a versioned schema (`"v": 1`). Empty path ⇒ `audit.log` in the run-log directory, following it live; an explicit path must be **absolute**. Rotates **daily by UTC date** to `audit.log.YYYYMMDD`; lifetime is the `auditLogFiles` retention knob (default 730 days), not a keep count. The **database is authoritative** — the file is an export of it, so `false` loses the stream, not the trail. Durable and not redacted by default — see `deploy/security-review.md`. |
| `AMADEUS_OIDC_ISSUER` / `_CLIENT_ID` / `_CLIENT_SECRET` / `_REDIRECT_URL` | — | OIDC relying party (A3.1/T8). Unset ⇒ login disabled (degraded). |
| `AMADEUS_SESSION_HASH_KEY` / `_BLOCK_KEY` | — | base64 32 bytes; unset ⇒ ephemeral keys (dev only) |
| `AMADEUS_COOKIE_SECURE` | `true` | set `false` for local HTTP |
| `AMADEUS_DEV_AUTH` | `false` | **dev only** — exposes `GET /auth/dev-login`, a one-click bypass that mints a synthetic admin session without the identity provider. Never enable in a deployed environment. |
| `AMADEUS_DEV_SEED` | `false` | **dev only** — loads representative demo data into an *empty* DB on boot so every view renders. No-op once the DB has data. |
| `AMADEUS_RUNNER_BOOTSTRAP_TOKEN` | — | shared runner registration token (A6.1) |
| `AMADEUS_KEK_FILE` / `AMADEUS_KEK` | — | stored-secret KEK: mounted file preferred, env fallback (S14). **Back up separately from the DB.** The `AMADEUS_SECRET_KEK*` aliases were removed in v1.5.41 — a deployment still setting them fails its first stored-secret operation with "no KEK configured". |
| `AMADEUS_RETENTION_RUNS_DAYS` / `_CHANGELOG_DAYS` / `_LOG_FILES_DAYS` | `90` / `365` / `90` | A4 retention **bootstrap defaults only** — they seed the `auditCompliance` settings blob on the first boot that finds it unset; afterwards the eleven per-table knobs in Settings → Audit & Compliance are authoritative (the knobs with no env var — `auditLogFiles`, `recycleBin`, `definitionRevisions`, `runnerPlacementHistory`, `archivedLogFiles` — start at their own defaults). `_LOG_FILES_DAYS` is new and reaps **on-disk run logs**, which nothing deleted before. |
| `AMADEUS_BACKUP_S3_BUCKET` | — | nightly VACUUM-INTO upload target (A4); empty disables upload |
| `AMADEUS_BACKUP_S3_ENDPOINT` / `_REGION` / `_ACCESS_KEY` / `_SECRET_KEY` / `_USE_SSL` | — / `us-east-1` / — / — / `true` | S3-compatible target (AWS/MinIO/Ceph/R2). See `deploy/backup-restore.md` |

> **Namespace note.** Names under the reserved prefixes `AMADEUS_VAR_`,
> `AMADEUS_SECRET_`, `AMADEUS_KEY_`, and `AMADEUS_RUN_` are **not** configuration —
> they are references the app resolves and injects into runs. Everything else
> under `AMADEUS_*` (the table above) is server/runner config. This is why the KEK
> now reads `AMADEUS_KEK*` rather than `AMADEUS_SECRET_KEK*`: it is config, not a
...
mounted unless `AMADEUS_DEV_AUTH=true`, so production never exposes it. The seed
...
| Path | Contents |
|---|---|
| `/var/lib/amadeus/amadeus.db` (+ `-wal`/`-shm`) | SQLite database (`AMADEUS_DB_PATH`) |
...
| `/var/lib/amadeus/logs/amadeus.log` (+ `amadeus.log.1` … `.N`) | **process log** — the same slog stream written to stdout, teed to disk so a crash is diagnosable where nothing collects stdout (`AMADEUS_LOG_FILE`). Mode `0640` in a `0750` directory. Rotated by size, not date; the generations don't end in `.log` and the live file is excluded by name, so the run-log reaper touches neither. **Not redacted** — see `deploy/security-review.md`. |
| `/var/lib/amadeus/logs/audit.log` (+ `audit.log.YYYYMMDD`) | compliance **audit stream** — JSON Lines, one record per audited event, rotated daily by UTC date (`AMADEUS_AUDIT_LOG`). Mode `0640` in a `0750` directory. The dated generations don't end in `.log`, so the run-log reaper skips them; the live file is excluded from it by name. Governed by the `auditLogFiles` retention knob (730 days), not `logFiles`. Also **not redacted**. |
| `/var/lib/amadeus/backups/` | nightly `VACUUM INTO` snapshots before S3 upload (A4) |
| `/var/lib/amadeus/git-cache/job-definitions/` | cached clone of the job-definitions repo (B3, `AMADEUS_GIT_CACHE_DIR`) |
...
three follow it unless `AMADEUS_LOG_FILE` / `AMADEUS_AUDIT_LOG` pin an absolute
path. Existing files are never moved.

The KEK for stored secrets is supplied **out of band** — a mounted secret file
(`AMADEUS_KEK_FILE`) or env (`AMADEUS_KEK`), **never** on this
...
- **CI-time validation (V1.1-1 / T11):** `deploy/ci-validate-template.yml` is the
  ...
  job included). Operator walkthrough — copy, pin `AMADEUS_IMAGE`, read a
